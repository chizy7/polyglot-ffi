...
from polyglot_ffi.ir.types import IRFunction, IRModule, IRType, TypeKind
...
class CStubGenerator:
        ...
        if return_type.is_primitive():
            ...
            if return_type.params and return_type.params[0].is_primitive():
                ...
                if inner_type.name == "string":
                    ...
                    lines.append("        int* result = (int*)malloc(sizeof(int));")
                    lines.append("        *result = Int_val(ml_some_value);")
                elif inner_type.name == "float":
                    lines.append("        double* result = (double*)malloc(sizeof(double));")
                    lines.append("        *result = Double_val(ml_some_value);")
                elif inner_type.name == "bool":
                    lines.append("        int* result = (int*)malloc(sizeof(int));")
                    lines.append("        *result = Bool_val(ml_some_value);")
                else:
    ...
    def _convert_c_list_to_ocaml(self, param_name: str, param_type: IRType) -> list:
        ...
        lines.append("    /* Convert C array to OCaml list */")
    ...
    def _convert_c_nested_list_to_ocaml(self, param_name: str, inner_list_type: IRType) -> list:
        ...
        lines.append("    /* Convert C nested list to OCaml list list */")
        lines.append(f"    void*** {param_name}_arr = (void***){param_name};")
        lines.append("    CAMLlocal1(inner_list);")
        lines.append("    CAMLlocal1(inner_cons);")
        lines.append("    CAMLlocal1(outer_cons);")
        lines.append(f"    ml_{param_name} = Val_emptylist;  /* Start with empty list */")
        lines.append(f"    for (int i = {param_name}_len - 1; i >= 0; i--) {{")
        lines.append("        /* Each element is [length, array_ptr] */")
        lines.append(f"        void** inner_pair = {param_name}_arr[i];")
        lines.append("        int inner_len = (int)(intptr_t)inner_pair[0];")

        # Convert inner array based on type
        if inner_element_type.name == "int":
            lines.append("        int* inner_arr = (int*)inner_pair[1];")
        elif inner_element_type.name == "float":
            lines.append("        double* inner_arr = (double*)inner_pair[1];")
        elif inner_element_type.name == "string":
            lines.append("        const char** inner_arr = (const char**)inner_pair[1];")
        elif inner_element_type.name == "bool":
            lines.append("        int* inner_arr = (int*)inner_pair[1];")

        # Build inner OCaml list
        lines.append("        inner_list = Val_emptylist;")
        lines.append("        for (int j = inner_len - 1; j >= 0; j--) {")
        lines.append("            inner_cons = caml_alloc(2, 0);")

        # Store element based on type
        if inner_element_type.name == "string":
            lines.append("            Store_field(inner_cons, 0, caml_copy_string(inner_arr[j]));")
        elif inner_element_type.name == "int":
            lines.append("            Store_field(inner_cons, 0, Val_int(inner_arr[j]));")
        elif inner_element_type.name == "float":
            lines.append("            Store_field(inner_cons, 0, caml_copy_double(inner_arr[j]));")
        elif inner_element_type.name == "bool":
            lines.append("            Store_field(inner_cons, 0, Val_bool(inner_arr[j]));")

        lines.append("            Store_field(inner_cons, 1, inner_list);")
        lines.append("            inner_list = inner_cons;")
        lines.append("        }")

        # Add inner list to outer list
        lines.append("        outer_cons = caml_alloc(2, 0);")
        lines.append("        Store_field(outer_cons, 0, inner_list);")
    ...
    def _convert_ocaml_list_to_c(self, return_type: IRType) -> list:
        ...
        if not return_type.params:
            ...
            lines.append("    void* result = (void*)ml_result;")
            lines.append("    CAMLreturnT(void*, result);")
            return lines
        ...
        lines.append("    /* Convert OCaml list to C array */")
        lines.append("    int list_len = 0;")
        lines.append("    value temp_list = ml_result;")
        lines.append("    while (temp_list != Val_emptylist) {")
        lines.append("        list_len++;")
        lines.append("        temp_list = Field(temp_list, 1);  /* tail */")
        lines.append("    }")
        lines.append("")

        # Allocate result struct (length + array pointer)
        lines.append("    /* Allocate result: [length, array_ptr] */")
        lines.append("    void** result = (void**)malloc(2 * sizeof(void*));")
        lines.append("    result[0] = (void*)(intptr_t)list_len;")
        lines.append("")

        # Handle empty list
        lines.append("    if (list_len == 0) {")
        lines.append("        result[1] = NULL;")
        lines.append("        CAMLreturnT(void*, result);")
        lines.append("    }")
        lines.append("")
        ...
        if element_type.name == "string":
            lines.append(
                "    const char** array = (const char**)malloc(list_len * sizeof(const char*));"
            )
        elif element_type.name == "int":
            lines.append("    int* array = (int*)malloc(list_len * sizeof(int));")
        elif element_type.name == "float":
            lines.append("    double* array = (double*)malloc(list_len * sizeof(double));")
        elif element_type.name == "bool":
            lines.append("    int* array = (int*)malloc(list_len * sizeof(int));")

        # Second pass: extract elements
        lines.append("")
        lines.append("    temp_list = ml_result;")
        lines.append("    for (int i = 0; i < list_len; i++) {")
        lines.append("        value head = Field(temp_list, 0);  /* head */")

        if element_type.name == "string":
            lines.append("        array[i] = strdup(String_val(head));")
        elif element_type.name == "int":
            lines.append("        array[i] = Int_val(head);")
        elif element_type.name == "float":
            lines.append("        array[i] = Double_val(head);")
        elif element_type.name == "bool":
            lines.append("        array[i] = Bool_val(head);")

        lines.append("        temp_list = Field(temp_list, 1);  /* tail */")
        lines.append("    }")
        lines.append("")
        lines.append("    result[1] = array;")
        lines.append("    CAMLreturnT(void*, result);")
    ...
    def _convert_ocaml_list_of_tuples_to_c(self, tuple_type: IRType) -> list:
        ...
        # First pass: count list length
        lines.append("    /* Convert OCaml list of tuples to C array */")
        lines.append("    int list_len = 0;")
        lines.append("    value temp_list = ml_result;")
        lines.append("    while (temp_list != Val_emptylist) {")
        lines.append("        list_len++;")
        ...
        # Allocate result struct (length + array pointer)
        lines.append("    /* Allocate result: [length, array_of_tuples] */")
        lines.append("    void** result = (void**)malloc(2 * sizeof(void*));")
        lines.append("    result[0] = (void*)(intptr_t)list_len;")
        lines.append("")
        ...
        lines.append("    void*** tuple_array = (void***)malloc(list_len * sizeof(void**));")
        lines.append("")

        # Second pass: extract tuples
        lines.append("    temp_list = ml_result;")
        lines.append("    for (int i = 0; i < list_len; i++) {")
        lines.append("        value tuple = Field(temp_list, 0);  /* head */")
        lines.append("")
        lines.append("        /* Allocate array for tuple elements */")
        ...
        lines.append("")
        lines.append("        tuple_array[i] = tuple_elem;")
        lines.append("        temp_list = Field(temp_list, 1);  /* tail */")
        lines.append("    }")
        lines.append("")
        lines.append("    result[1] = tuple_array;")
        lines.append("    CAMLreturnT(void*, result);")
    ...
    def _convert_c_tuple_to_ocaml(self, param_name: str, param_type: IRType) -> list:
        ...
        lines.append("    /* Convert C tuple to OCaml tuple */")
        lines.append(f"    void** {param_name}_arr = (void**){param_name};")
    ...
    def _convert_ocaml_tuple_to_c(self, return_type: IRType) -> list:
        ...
        lines.append("    /* Convert OCaml tuple to C array */")
